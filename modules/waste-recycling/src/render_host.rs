//! Host dashboard surfaces.

use portaki_sdk::prelude::*;
use portaki_sdk::sdui::action::Action;
use portaki_sdk::sdui::primitives::{Button, Field, Form, Page, Text, TextArea};
use portaki_sdk::sdui::surface::Surface;
use serde_json::json;

use crate::config::load_config;

/// Host configuration page (bins JSON + collection schedule).
#[portaki_sdk::surface(host, id = "main")]
pub fn render_host_main(ctx: HostContext) -> Surface {
    let _ = ctx;
    let config = load_config().unwrap_or_default();

    let submit_args = json!({
        "bins_json": config.bins_json,
        "collection_schedule": config.collection_schedule,
    });
    let save_action = serde_json::to_value(Action::command(
        "waste-recycling",
        "updateConfig",
        submit_args,
    ))
    .unwrap_or(json!({}));

    Surface::new(
        Page::new()
            .title(json!("i18n:surface.host.main.title"))
            .child(
                Text::new()
                    .text(json!("i18n:surface.host.main.subtitle"))
                    .variant(json!("body")),
            )
            .child(
                Form::new()
                    .child(
                        Field::new()
                            .name(json!("bins_json"))
                            .label(json!("i18n:host.bins.label"))
                            .child(
                                TextArea::new()
                                    .name(json!("bins_json"))
                                    .value(json!(config.bins_json))
                                    .placeholder(json!("i18n:host.bins.placeholder")),
                            ),
                    )
                    .child(
                        Field::new()
                            .name(json!("collection_schedule"))
                            .label(json!("i18n:host.schedule.label"))
                            .child(
                                TextArea::new()
                                    .name(json!("collection_schedule"))
                                    .value(json!(config.collection_schedule))
                                    .placeholder(json!("i18n:host.schedule.placeholder")),
                            ),
                    )
                    .child(
                        Text::new()
                            .text(json!("i18n:host.main.help"))
                            .variant(json!("caption")),
                    )
                    .child(
                        Button::new()
                            .label(json!("i18n:host.save"))
                            .action(save_action),
                    ),
            ),
    )
    .with_id("main")
}
